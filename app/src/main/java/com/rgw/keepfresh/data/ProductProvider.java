...
public class ProductProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        // Check that the barcode is not null
        String barcode = values.getAsString(ProductEntry.COLUMN_PRODUCT_BARCODE);
        if (barcode == null) {
            throw new IllegalArgumentException("Product requires valid barcode");
        }
        int day = values.getAsInteger(ProductEntry.COLUMN_DATE_DAY);
        if (day == 0) {
            throw new IllegalArgumentException("Product Requires an Expiration Date");
        }
        String url = values.getAsString(ProductEntry.COLUMN_PRODUCT_IMAGEURL);
